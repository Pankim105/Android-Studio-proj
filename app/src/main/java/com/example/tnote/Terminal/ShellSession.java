   ...
   // private OutputListener outputListener;  // 输出监听器
    ...
    private boolean shellExec(String command) {
        ...
        if (command.equals("cd ~")) {
            command = "cd " + MainActivity.getAppDir();
        }
        else if(command.startsWith("cd ..")) {
            command = handleParentDirectory(command);
        }
        try {
            // 写入命令并刷新
    ...
    private void handleCdCommand(String command) {
        try {
            ...
            if (newDir.isDirectory()&&newDir.canRead()) {
                FileBrowserFragment.setCurrentDirectory(newDir);
                notifyDirectoryChanged();
            }
            else if (newDir.isDirectory()&&!newDir.canRead()) {
                Log.println(Log.INFO,"currDir check",newDir.getAbsolutePath());
                FileBrowserFragment.setCurrentDirectory(newDir);
                notifyDirectoryChanged();
                //shellExec("cd "+MainActivity.getAppDir());
            }
        } catch (IOException e) {
     ...
     */
    private String handleParentDirectory(String command) {
        if (command.equals("cd ..")) {
            command="cd "+FileBrowserFragment.getCurrentDirectory().getParentFile();
            return command;
        }
        String relativeDir = command.split("/", 2)[1];
        Log.i(TAG, "上级目录路径: " + relativeDir);
        File parentDir = FileBrowserFragment.getCurrentDirectory().getParentFile();
        if (parentDir != null) {
            command = "cd " + parentDir.getAbsolutePath() + "/" + relativeDir;
            return command;
        }
        else return FileBrowserFragment.getCurrentDirectory().getAbsolutePath();
    }
    ...
    private void notifyDirectoryChanged() {
        mainHandler.post(() -> {
            ...
            if (activity != null && !activity.isDestroyed()) {
                ...
                if (fragment != null) {
                    Log.println(Log.INFO,"currDir check",FileBrowserFragment.getCurrentDirectory().getAbsolutePath());
                    fragment.loadDirectory(FileBrowserFragment.getCurrentDirectory()); // 更新文件浏览器目录
                }
            }
        });
    }
